...
import { createPushNotice, uploadImageToGitHub } from '../../actions';
import { Loader2, ArrowLeft } from 'lucide-react';
import Link from 'next/link';

export default function PushNoticeEditor() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createPushNotice(formData as any);
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Create New Push Notice</h1>
            <p className="text-muted-foreground mt-1">Create a push notice for the homepage</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <Input 
                ...
                onChange={e => setFormData({...formData, display_until: e.target.value ? e.target.value : null})}
                className="h-12 text-base"
              />
            </div>
            ...
            <div className="flex gap-4 pt-4 border-t border-border">
              <Button type="submit" disabled={isSubmitting || isUploading} className="flex-1 h-12 text-base font-semibold">
                {isSubmitting ? <Loader2 className="animate-spin mr-2" /> : 'Create Push Notice'}
              </Button>
